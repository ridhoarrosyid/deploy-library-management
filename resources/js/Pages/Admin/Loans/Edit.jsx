...
export default function Edit(props) {
  const { data, setData, reset, post, processing, errors } = useForm({
    user: props.loan.user.name ?? null,
    book: props.loan.book.title ?? null,
    loan_date: props.loan.loan_date,
    due_date: props.loan.due_date,
    _method: props.page_settings.method,
  });

  const formatedLoanDate = new Date(data.loan_date).toLocaleDateString('id-ID', { dateStyle: 'long' });
  const formatedDueDate = new Date(data.due_date).toLocaleDateString('id-ID', { dateStyle: 'long' });
          ...
          <form onSubmit={onHandleSubmit} className="space-y-6">
            <div className="grid w-full items-center gap-1.5">
              <p className="font-semibold">
                Buat pinjaman dari tanggal {formatedLoanDate} sampai {formatedDueDate}
              </p>
            </div>
