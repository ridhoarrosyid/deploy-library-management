...
export default function Index(props) {
  ...
  useFilter({ route: route('admin.route-accesses.index'), values: params, only: ['route_accesses'] });
  return (
    <div className="flex w-full flex-col pb-32">
      <div className="mb-8 flex flex-col items-start justify-between gap-y-4 lg:flex-row lg:items-center">
        <HeaderTitle title={props.page_settings.title} subtitle={props.page_settings.subtitle} icon={IconRoute} />
      ...
      <Card>
          ...
          <Table className="w-full">
            <TableHeader>
              <TableRow>
                ...
                <TableHead>
                  {' '}
                  <Button className="group inline-flex" variant="ghost" onClick={() => onSortable('route_name')}>
                    Route
                    <span className="ml-2 flex-none rounded text-muted-foreground">
                      <IconArrowDownUp className="size-4 text-muted-foreground" />
                    </span>
                ...
                <TableHead>
                  {' '}
                  <Button className="group inline-flex" variant="ghost" onClick={() => onSortable('role_id')}>
                    Peran
                    <span className="ml-2 flex-none rounded text-muted-foreground">
                      <IconArrowDownUp className="size-4 text-muted-foreground" />
                    </span>
                ...
                <TableHead>
                  {' '}
                  <Button className="group inline-flex" variant="ghost" onClick={() => onSortable('permission_id')}>
                    Izin
                    <span className="ml-2 flex-none rounded text-muted-foreground">
                      <IconArrowDownUp className="size-4 text-muted-foreground" />
                    </span>
            ...
            <TableBody>
              {route_accesses.map((route_access, i) => (
                <TableRow key={i}>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() =>
                                router.delete(route('admin.route-accesses.destroy', [route_access]), {
                                  preserveScroll: true,
                                  preserveState: true,
                                  onSuccess: (success) => {
        ...
        <CardFooter className="flex w-full flex-col items-center justify-between border-t py-2 lg:flex-row">
          <p className="mb-2 text-sm text-muted-foreground">
            Manampilkan <span className="font-medium text-orange-500"> {meta.from ?? 0}</span> dari {meta.total} rute
            akses
          </p>
