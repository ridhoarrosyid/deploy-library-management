...
import IconCalendarMonth from '@/Components/icons/IconCalendarMonth';
  ...
  return (
    <div className="flex w-full flex-col space-y-4 pb-32">
      <div className="flex flex-col items-start justify-between gap-y-4 lg:flex-row lg:items-center">
        <HeaderTitle title={props.page_settings.title} subtitle={props.page_settings.subtitle} icon={IconMoneybag} />
      </div>
      ...
      <Card>
        ...
        <CardFooter className="flex w-full flex-col items-center justify-between border-t py-2 lg:flex-row">
          <p className="mb-2 text-sm text-muted-foreground">
            Manampilkan <span className="font-medium text-orange-500"> {meta.from ?? 0}</span> dari {meta.total} denda
          </p>
      ...
      <div className="grid gap-4 md:gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Member Yang Memilik Denda Paling Banyak</CardTitle>
          </CardHeader>
          <CardContent className="px-0 py-0 [&_td]:whitespace-nowrap [&_td]:px-6 [&_th]:px-6">
            <Table className="w-full">
              <TableHeader>
              ...
              <TableBody>
                {props.page_data.most_fine_members.map((most_fine_member, i) => (
                  <TableRow key={i}>
                    <TableCell> {i + 1} </TableCell>
                    <TableCell>{most_fine_member.user.name}</TableCell>
                    <TableCell>{formatToRupiah(most_fine_member.total_fee)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
