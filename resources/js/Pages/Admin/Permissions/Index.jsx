...
export default function Index(props) {
  ...
  useFilter({ route: route('admin.permissions.index'), values: params, only: ['permissions'] });
  return (
    <div className="flex w-full flex-col pb-32">
      <div className="mb-8 flex flex-col items-start justify-between gap-y-4 lg:flex-row lg:items-center">
        <HeaderTitle title={props.page_settings.title} subtitle={props.page_settings.subtitle} icon={IconVersions} />
      ...
      <Card>
          ...
          <Table className="w-full">
            <TableHeader>
              <TableRow>
                ...
                <TableHead>
                  {' '}
                  <Button className="group inline-flex" variant="ghost" onClick={() => onSortable('guard_name')}>
                    Guard
                    <span className="ml-2 flex-none rounded text-muted-foreground">
                      <IconArrowDownUp className="size-4 text-muted-foreground" />
                    </span>
        ...
        <CardFooter className="flex w-full flex-col items-center justify-between border-t py-2 lg:flex-row">
          <p className="mb-2 text-sm text-muted-foreground">
            Manampilkan <span className="font-medium text-orange-500"> {meta.from ?? 0}</span> dari {meta.total} izin
          </p>
