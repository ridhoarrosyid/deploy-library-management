...
class RouteAccessSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $admin_routes = collect(Route::getRoutes())->filter(function ($route) {
            return str_starts_with($route->getName(), 'admin.') || str_starts_with($route->getName(), 'profile.') || $route->getName() === 'dashboard';
        });
        ...
        $operator_routes = collect(Route::getRoutes())->filter(function ($route) use ($operator_prefixes) {
            return in_array($route->getName(), ['dashboard']) || collect($operator_prefixes)->contains(function ($prefix) use ($route) {
        ...
        $member_routes = collect(Route::getRoutes())->filter(function ($route) {
            return str_starts_with($route->getName(), 'front.') || str_starts_with($route->getName(), 'profile.') || $route->getName() === 'dashboard';
        });
